...
def train_model(dataset='hmdb51', num_classes=51, lr=1e-4, num_epochs=100, batch_size=8, clip_len=16, patience=20):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_dataloader:
            ...
            train_top5_corrects += top_k_accuracy(outputs, labels, k=5) * inputs.size(0)
        
        train_loss = train_loss / len(train_dataloader.dataset)
        train_acc = train_corrects.double() / len(train_dataloader.dataset)
        train_top5_acc = train_top5_corrects / len(train_dataloader.dataset)
        logger.info(f"Epoch {epoch + 1}/{num_epochs} | Train Loss: {train_loss:.4f} Acc: {train_acc:.4f} Top-5 Acc: {train_top5_acc:.4f}")

        # Validation
        model.eval()
        val_loss = 0.0
        val_corrects = 0.0
        val_top5_corrects = 0.0
        with torch.no_grad():
            for inputs, labels in val_dataloader:
                ...
                val_top5_corrects += top_k_accuracy(outputs, labels, k=5) * inputs.size(0)
        
        val_loss = val_loss / len(val_dataloader.dataset)
        val_acc = val_corrects.double() / len(val_dataloader.dataset)
        val_top5_acc = val_top5_corrects / len(val_dataloader.dataset)
        logger.info(f"Epoch {epoch + 1}/{num_epochs} | Val Loss: {val_loss:.4f} Acc: {val_acc:.4f} Top-5 Acc: {val_top5_acc:.4f}")
    ...
    with torch.no_grad():
        for inputs, labels in test_dataloader:
            ...
            test_top5_corrects += top_k_accuracy(outputs, labels, k=5) * inputs.size(0)
    
    test_loss = test_loss / len(test_dataloader.dataset)
    test_acc = test_corrects.double() / len(test_dataloader.dataset)
    test_top5_acc = test_top5_corrects / len(test_dataloader.dataset)
    logger.info(f"Test Loss: {test_loss:.4f} Acc: {test_acc:.4f} Top-5 Acc: {test_top5_acc:.4f}")
