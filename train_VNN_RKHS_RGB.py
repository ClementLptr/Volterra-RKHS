...
def parse_args():
    ...
    parser.add_argument('--num_projectors', type=int, default=896, help='Number of projectors to use')
    return parser.parse_args()

def compute_projectors(num_projectors=896, projectors_dir="results/projectors", 
...
def initialize_models(num_classes=51, num_projectors=896, projectors_dir="results/projectors", 
                      device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
...
def initialize_optimizer(model_RGB, lr=1e-4):
    ...
    optimizer = optim.AdamW(train_params, lr=lr, weight_decay=1e-2)  # Or optim.SGD(train_params, lr=lr, momentum=0.9, weight_decay=1e-2)
    return optimizer
def initialize_scheduler(optimizer, num_epochs, warmup_epochs=5, min_lr=1e-6):
    ...
    for i, (inputs, labels) in enumerate(trainval_loaders[phase]):
        try:
            ...
            if phase == 'train':
                ...
                torch.nn.utils.clip_grad_norm_(model_RGB.parameters(), max_norm=1.0)  # Clip gradients
...
def train_model(dataset='hmdb51', save_dir=None, num_classes=51, lr=1e-4,
                ...
                warmup_epochs=5, save_csv=True, num_projectors=896, patience = 10):
    save_dir = save_dir or os.path.dirname(os.path.abspath(__file__))    
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    logger.info(f"Using device: {device}")
    
    modelName = 'RKHS_Five_Layers'
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            if val_metrics['loss'] < best_val_loss:
                ...
                patience_counter = 0
                save_checkpoint(epoch, model_RGB, optimizer, scheduler, save_dir, saveName, model_version)
            else:
                patience_counter += 1
